...
**User Guide**
- Submit a GitHub repo URL from the UI.
- Watch status move through `QUEUED → FETCHING → FETCHED → REVIEWING → COMPLETED`.
- Select a completed job to see review answers.

**Troubleshooting**
- `Failed to submit repo`: Intake service not running or CORS blocked.
- `No active scans yet` with no errors: MongoDB or RabbitMQ not reachable by services.
- Jobs stuck in `FETCHING`: repo-fetcher worker not running or workspace volume missing.
- Jobs stuck in `REVIEWING`: vibe-review worker not running or review timeout.
